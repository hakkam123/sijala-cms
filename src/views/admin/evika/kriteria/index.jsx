...
const KriteriaManagement = () => {
  ...
  const [data, setData] = useState([
    ...
    {
      id: 2,
      nama: "Kriteria Sosial Ekonomi",
      bobot: 0.25,
      created_at: "2024-01-15",
    },
    {
      id: 3,
      nama: "Kriteria Kelembagaan",
      bobot: 0.25,
      created_at: "2024-01-15",
    },
    {
      id: 4,
      nama: "Kriteria Governance",
      bobot: 0.20,
      created_at: "2024-01-15",
    },
  ]);
        ...
        <div className="mb-5">
          ...
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-navy-700 dark:text-white">
              Manajemen Kriteria EVIKA
            </h2>
            <button
              onClick={() => navigate("/admin/evika/kriteria/create")}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 dark:border-navy-600">
                ...
                <th className="text-left py-3 px-4 text-navy-700 dark:text-white">Nama Kriteria</th>
                <th className="text-left py-3 px-4 text-navy-700 dark:text-white">Bobot</th>
                <th className="text-left py-3 px-4 text-navy-700 dark:text-white">Tanggal Dibuat</th>
            ...
            <tbody>
              {filteredData.map((item, index) => (
                <tr key={item.id} className="border-b border-gray-200 dark:border-navy-600">
                  ...
                  <td className="py-3 px-4 text-navy-700 dark:text-white">{item.nama}</td>
                  <td className="py-3 px-4 text-navy-700 dark:text-white">{(item.bobot * 100).toFixed(0)}%</td>
                  <td className="py-3 px-4 text-navy-700 dark:text-white">{item.created_at}</td>
                  <td className="py-3 px-4">
                    <div className="flex justify-center gap-2">
                      <button
                        onClick={() => navigate(`/admin/evika/kriteria/edit/${item.id}`)}
...
export default KriteriaManagement;
